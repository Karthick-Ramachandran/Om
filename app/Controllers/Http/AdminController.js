...
class AdminController {
  ...
  async datatable({ view }) {
    const data = await Upload.query()
      .where("where", "repo")
      .fetch();
    return view.render("dashboard.datatable", { data: data.toJSON() });
  }
  async upload({ view }) {
    return view.render("dashboard.upload");
  }
  async mapping({ view }) {
    return view.render("dashboard.mapping");
  }
  ...
  async wanted({ view }) {
    const data = await Upload.query()
      .where("where", "wanted")
      .fetch();
    return view.render("dashboard.wanted", { data: data.toJSON() });
  }
}
